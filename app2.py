...
def calculate_combined_complementary_ranges(cidrs: List[str]) -> List[str]:
    try:
        ...
        # Combine overlapping or contiguous ranges into continuous blocks
        merged_ranges = []
        current_start, current_end = covered_ranges[0]

        for start, end in covered_ranges[1:]:
            if start <= current_end + 1:  # Overlap or contiguous
        ...
        # Check if merged ranges cover the entire IP space
        if len(merged_ranges) == 1 and merged_ranges[0] == (
            0,
            int(ipaddress.IPv4Address("255.255.255.255")),
        ):
            # If the ranges cover the entire IPv4 space, no complement exists
            return []
